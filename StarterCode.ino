...
void loop()
{
  ...
  static float errorR = 0;      // this is the accumulated velocity error in m/s
  static float errorL = 0;
...
//        Serial.print("\t");
        Serial.println(angle_rad_accum,5);
//        Serial.println(testSpeed);
  ...
  angle_rad = ((float)angle)/1000/180*3.14159 - del_theta - 0.12;
  ...
  if(cur_time - prev_time > UPDATE_TIME_MS && start_flag)
  {
    ...
    angle_rad_accum = angle_rad_accum + angle_rad * delta_t;

    dV = Rp * angle_rad + Ri * angle_rad_accum;
    errorR = dV - vR;
    errorL = dV - vL;

    error_right_accum = error_right_accum + errorR * delta_t;
    error_left_accum = error_left_accum + errorL * delta_t;

    PWM_left = Kp * errorL + Ki * error_left_accum;
    PWM_right = Kp * errorR + Ki * error_right_accum;
